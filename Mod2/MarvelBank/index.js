const marvelMovies = {
  ...
  // #1. Return an array of comic titles that were written before 2000
function before2000(comics) {
  comicTitles = [];
  console.log(comics)
  for (const comic in comics) {
    if comic["publishDate"] > 2000 (
  ...
  // #2. Return an array of comics that were edited by Stan Lee
  
  
  // #3. Return an array of comics that were later adapted
  
  
  // #4. Return an array of comics and an array of movies (concated) that feature Captain America
  
  
  // #5. Return an array of movies with an R rating
  
  
  // #6. Return an array of movies with above a 8 rating on imdb
  
  
  // #7. Sort the movies by their imdb ratings
  
  
  // #8. Create an object with each character as the key and the movies they appear in as the value
